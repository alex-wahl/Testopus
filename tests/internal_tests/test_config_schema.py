...
from core.config.schema import RootConfig, ToolshopConfig
...
def test_schema_allows_extra_keys():
    cfg = {
        "configuration": {"toolshop": {"web_url": "https://x.test", "extra": 1}},
        "startrack": {"enabled": True},
    }
    RootConfig.model_validate(cfg)  # extra top-level + extra toolshop keys allowed


def test_schema_rejects_toolshop_without_web_url():
    with pytest.raises(ValidationError):
        ToolshopConfig.model_validate({"api_url": "https://api.x.test"})
...
def test_load_config_from_cli_accepts_valid(tmp_path, monkeypatch):
    ...
    assert cfg["configuration"]["toolshop"]["web_url"] == "https://x.test"
