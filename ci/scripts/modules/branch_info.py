...
def add_branch_info(report_dir: str, custom_branch: str = None) -> None:
    ...
    logger.info(
        f"Added branch information: {branch} at the top of the environment table"
    )
    ...
    try:
        ...
        if isinstance(env_data, list):
            ...
            env_data = [
                ...
                if not (isinstance(item, dict) and item.get("name") == "Branch")
            ]

            # ALWAYS add Branch at position 0
            env_data.insert(0, {"name": "Branch", "values": [branch]})
    ...
    for html_file in html_files:
        try:
            ...
            if modified:
                ...
                if not (
                    "<tr><td>Branch</td><td>" in new_content and "</tr>" in new_content
                ):
                    logger.warning(
                        f"Skipping HTML update for {html_file} - could result in malformed HTML"
                    )
                    continue
