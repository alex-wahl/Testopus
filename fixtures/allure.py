...
from utils.ci_env import resolve_branch, resolve_provenance
...
def pytest_runtest_teardown(item):
    ...
    try:
        ...
        # Scrub sensitive inputs (e.g. password fields) before capturing the DOM so
        # credentials never reach a published screenshot. Fail-open.
        try:
        ...
        logger.warning("Failed to capture screenshot: %s", redact(str(e)))
...
def pytest_sessionfinish(session):
    ...
    # Branch + CI run provenance (shared helper; see utils/ci_env.py)
    ...
    # Ensure the results dir exists: setup_directories is a test-scoped fixture, so it never runs
    # when no tests execute (e.g. `pytest --collect-only`), yet this session-finish hook always
    # does. Fail-safe, mirrors setup_directories.
    ...
    with open(env_file, "w") as f:
        ...
        f.write(f"Commit={provenance['commit']}\n")
        f.write(f"RunId={provenance['run_id']}\n")
        f.write(f"RunAttempt={provenance['run_attempt']}\n")
        f.write(f"CiUrl={provenance['ci_url']}\n")
